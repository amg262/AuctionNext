...
public class AuctionBusTests : IAsyncLifetime
{
	...
	{
		// Arrange: Create a valid auction DTO and set authentication token for the HTTP client.
		//var auction = GetAuctionForCreate();
		var auction = DbHelper.GetAuctionForCreate();
		_httpClient.SetFakeJwtBearerToken(AuthHelper.GetBearerForUser("bob"));

		// Act: Attempt to create an auction via the API.
		var response = await _httpClient.PostAsJsonAsync("api/auctions", auction);

		// Assert: Verify that the response indicates success and that an AuctionCreated event was published on the bus.
